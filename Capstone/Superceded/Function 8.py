...
for i in range(int(1 / step)):
    ...
    num_pred = 2
    x1 = np.linspace(start, end, num_pred)
    x2 = np.linspace(start, end, num_pred)
    x3 = np.linspace(start, end, num_pred)
    x4 = np.linspace(start, end, num_pred)
    x5 = np.linspace(start, end, num_pred)
    x6 = np.linspace(start, end, num_pred)
    x7 = np.linspace(start, end, num_pred)
    x8 = np.linspace(start, end, num_pred)
    ...
    for k, v in enumerate(y_mean):
        means.append(v)

    for k, v in enumerate(y_std):
        stds.append(v)
...
if af_option == 'PI': # Probability of Improvement (PI)
    af = norm.cdf(numerator, means, stds)

elif af_option == 'UB': # Upper Bound
    af = means + 5 * stds

elif af_option == 'EI': # Expected Improvement (EI)
    af = (means - numerator) * norm.cdf(numerator, means, stds) + stds * norm.pdf(numerator, means, stds)

next_point = xvals[np.argmax(af)]
print(next_point)
